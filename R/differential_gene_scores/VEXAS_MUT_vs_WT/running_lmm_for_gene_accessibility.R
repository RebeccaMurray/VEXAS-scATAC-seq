...
lapply(c("HSC", "EMP", "LMPP", "MkP", "GMP", "CD14 Mono"), function(celltype) {
# lapply(c("HSC"), function(celltype) {
  ...
  ## Subset RNA object for cluster, keep genes expressed in 5%
  ...
  m <- se@assays$RNA_ArchR@data[intersect(genes.to.test, rownames(se@assays$RNA_ArchR@data)), ]
  ...
  print("Saving output...")
  write_delim(diffLMM.results, file = paste0("data/differential_gene_scores_ArchR/VEXAS_MUT_vs_WT/", celltype, "_DiffLMM2_expressed_only_10p_RNA_obj_donor_filtered.csv"), delim = ",")
  print("Done!")
})
